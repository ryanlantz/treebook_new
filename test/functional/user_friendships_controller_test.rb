...
class UserFriendshipsControllerTest < ActionController::TestCase
	context "#index" do
		...
		context "when logged in" do
			setup do
				@friendship1 = create(:pending_user_friendship, user: users(:george), friend: create(:user, first_name: 'Pending', last_name: 'Friend'))
				@friendship2 = create(:accepted_user_friendship, user: users(:george), friend: create(:user, first_name: 'Active', last_name: 'Friend'))
				@friendship3 = create(:requested_user_friendship, user: users(:george), friend: create(:user, first_name: 'Requested', last_name: 'Friend'))
				@friendship4 = user_friendships(:blocked_by_george)
			...
			should "display date information on a pending friendship" do
	...
	context "#new" do
		...
		context "when logged in" do
			...
			should "display the friend's name" do
				get :new, friend_id: users(:emilly)
				assert_match /#{users(:emilly).full_name}/, response.body
			end
			...
			should "assign a new user friendship to the correct friend" do
				get :new, friend_id: users(:emilly)
				assert_equal users(:emilly), assigns(:user_friendship).friend
			end


			should "assign a new user friendship to the currently logged in user" do
				get :new, friend_id: users(:emilly)
				assert_equal users(:george), assigns(:user_friendship).user
			end
			...
			should "ask if you really want to friend the user" do
				get :new, friend_id: users(:emilly)
				assert_match /Do you really want to friend #{users(:emilly).full_name}?/, response.body
			end
		end
	end
	...
	context "#create" do
		...
		context "when logged in" do
			...
			context "successfully" do
				should "create two user friendship objects" do
					assert_difference 'UserFriendship.count', 2 do
						post :create, user_friendship: { friend_id: users(:emilly).profile_name }
					end
				end
			end
			...
			context "with a valid friend_id" do
				setup do
					post :create, user_friendship: { friend_id: users(:emilly) }
				end

				should "assign a friend object" do
					assert assigns(:friend)
					assert_equal users(:emilly), assigns(:friend)
				end
				...
				should "redirect to the profile page of the friend" do
					assert_response :redirect
					assert_redirected_to profile_path(users(:emilly))
				end
	...
	context "#accept" do
		context "#create" do
			...
			context "when logged in" do
				setup do
					@friend = create(:user)
					@user_friendship = create(:pending_user_friendship, user: users(:george), friend: @friend)
					create(:pending_user_friendship, friend: users(:george), user: @friend)
					sign_in users(:george)
				end
  ...
  context "#edit" do
	...
	context "#destroy" do
		...
		context "when logged in" do
			setup do
				@friend = create(:user)
				@user_friendship = create(:accepted_user_friendship, friend: @friend, user: users(:george))
				create(:accepted_user_friendship, friend: users(:george), user: @friend)
